...
export class OfferService {
  ...
  public async listOffers(
    userId: string,
    listOffersParams: ListOffersParams
    ...
    // paginação
    const paginatedOffers = await this.getPaginatedListedOffers(page, limit);
  ...
  public async unlistOffer(userId: string, offerId: string): Promise<void> {
  ...
  private async checkIfUserOwnsWallet(userId: string, offerWalletId: string) {
  ...
  private async checkEnoughBalance(createOfferDTO: CreateOfferDTO) {
    ...
    const totalOffered = await this.getTotalOfferedByAsset(
      walletId,
      currencyId
    );
  ...
  private isOfferAmountMoreThanOwned(
    offerAmount: number,
    asset: Asset,
    totalOffered: number
  ) {
  ...
  private async createOfferFromDTO(
    userId: string,
    createOfferDTO: CreateOfferDTO
  ) {
  ...
  private checkIfUserOwnsOffer(userId: string, offer: Offer) {
