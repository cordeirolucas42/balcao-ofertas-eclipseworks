...
import {
  ...
  offersCreatedToday,
  offersListed,
  offerPropsToPopulate,
  recentOffersFirst,
  offersCreatedBy
} from '../model/offer.schema';
...
describe('OfferService', () => {
  ...
  describe('listOffers', () => {
    test('Listagem por paginação', async () => {
      ...
      expect(offerMocks.offerModel.populate).toBeCalledWith(
        offerPropsToPopulate
      );
  ...
  describe('createOffer', () => {
    test('Criar oferta corretamente', async () => {
      ...
      expect(offerMocks.walletModel.findById).toBeCalledWith(
        offerMocks.walletId
      );
      expect(offerMocks.currencyModel.findById).toBeCalledWith(
        offerMocks.currencyId
      );
      ...
      // checkEnoughBalance
      expect(offerMocks.assetModel.findOne).toBeCalledWith(
        offerMocks.findAsset
      );
      ...
      // createOfferFromDTO
      expect(offerMocks.offerModel.create).toBeCalledWith(
        offerMocks.offerToCreate
      );
    ...
    test('WalletId precisa corresponder a uma carteira', async () => {
      ...
      expect(res).rejects.toThrow(
        new NotFoundException('There is no document with Id walletId')
      );
    ...
    test('CurrencyId precisa corresponder a uma carteira', async () => {
      ...
      expect(res).rejects.toThrow(
        new NotFoundException('There is no document with Id currencyId')
      );
    ...
    test('Somente dono da carteira pode criar oferta com ela', async () => {
      ...
      expect(res).rejects.toThrow(
        new UnauthorizedException('User does not own wallet')
      );
    ...
    test('Máximo de 5 ofertas por dia por usuário', async () => {
      ...
      expect(res).rejects.toThrow(
        new ForbiddenException(
          'Maximum amount of offers created per day reached'
        )
      );
  ...
  describe('unlistOffer', () => {
    test("Deve setar prop 'listed' da oferta (soft-delete)", async () => {
      ...
      await offerService.unlistOffer(offerMocks.userId, offerMocks.offerId);
    ...
    test('OfferId precisa corresponder a uma oferta', async () => {
      ...
      expect(res).rejects.toThrow(
        new NotFoundException('There is no offer with Id offerId')
      );
    });
    test('Somente criador da oferta pode soft-delete', async () => {
      ...
      expect(res).rejects.toThrow(
        new UnauthorizedException('User does not own offer')
      );
    });
  });
});
