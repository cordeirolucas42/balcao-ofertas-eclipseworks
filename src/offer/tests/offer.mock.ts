...
const foundOffers = [{ amount: 1 }, { amount: 1 }, { amount: 1 }];
...
const createOfferCorrectlyModelMocks = () => {
  ...
  offerMocks.currencyModel.exec = jest
    .fn()
    .mockResolvedValue(offerMocks.currency);

  offerMocks.assetModel.exec = jest.fn().mockResolvedValue(offerMocks.asset);

  offerMocks.offerModel.create = jest
    .fn()
    .mockResolvedValue(offerMocks.createdOffer);
};
